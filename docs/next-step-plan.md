...
- ✅ 2026-03-24 已修复 release workflow 的 `.app` artifact 传递方式：`release-desktop.yml` 现先用 `ditto -c -k --keepParent` 归档 `Joudo.app`，`build-dmg` job 再显式解包恢复 app bundle，避免直接下载目录型 artifact 时丢失 `Joudo.app` 顶层路径，导致 `build-dmg.mjs` 报 `Missing app bundle`
- ✅ 2026-03-25 已修复 GitHub Actions 构建架构漂移：`desktop-macos.yml` 与 `release-desktop.yml` 现固定使用 `macos-15-intel` runner，避免 `macos-latest` 构建出 `aarch64` 桌面包后，在当前 x64 Ventura 开发机上提示“不支持此应用程序”
- ✅ 2026-03-25 已把 release workflow 扩展为双架构产物：`release-desktop.yml` 现会分别在 `macos-13` 与 `macos-14` 上构建 `x64` / `arm64` 的 `.app` 与 `.dmg`，artifact 名称显式带架构后缀，避免下载后无法区分目标机器
- ✅ 2026-03-25 已补强 packaged desktop 回归脚本在 GitHub Actions 上的启动方式：`validate-packaged-runtime.mjs` 现会在 CI 中直接拉起 app bundle 内可执行文件并采集 stdout/stderr 日志，避免 `open -n Joudo.app` 在 runner 上未稳定启动 app 时只留下 `bridge health check timed out` 的空错误
